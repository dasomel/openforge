...
`portfolio/agent-audit.json` (the per-repository false-green, deterministic-control, and agent-skill-maturity audit) is a fifth `dashboard.json` input, folded into an additive top-level `agent_audit` key by `render_dashboard_json`/`build_agent_audit_view`. It does not change `version`: the consumer parses the payload structurally and ignores unknown top-level keys, so an added key is backward compatible while a version bump would not be. `agent_audit.repositories[]` mirrors the checked-in audit order (already priority-sorted). Two fields — `swallowed_failure_count` and `local_agent_ci_gate` — are still landing on the audit matrix and are absent from some or all repository records today; the dashboard renders that absence as `null`, never as `0`/`false`, because `0` asserts "measured, none found" while the truth is "not measured at this revision". The matching `agent_audit.summary` fields (`swallowed_failure_findings`, `repositories_with_local_ci_gate`) sum only the repositories that actually carry the key and are themselves `null` when no repository does; `swallowed_failure_findings_measured_repositories` and `local_ci_gate_measured_repositories` report how many repositories contributed, so a reader can tell partial coverage from full coverage. `portfolio/agent-audit.json` is a required input like the other four: a missing or unparseable file fails the generator loudly rather than shipping an empty `agent_audit` block.
