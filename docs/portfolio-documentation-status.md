...
## 2026-09-12 documentation freshness pass (DOC-010)

Issue #74 asks for priority OSS/`main` claims to be audited against current implementation. This
section records the `DOC-010` result -- whether a repository carries a dated
`docs/IMPLEMENTATION-STATUS.md` snapshot verified against `main`. It supplements the manual
adoption-journey review above; nothing above is retracted.

**Read the default branch, not the working tree.** The first attempt at this pass ran the auditor
over local sibling clones and reported that Narwhal Portal, Kube-Ready-Box and ClusterDeck had no
`docs/IMPLEMENTATION-STATUS.md` at all. That was wrong. Every one of those clones was between 5
and 12 commits behind its remote, and all three carry the file on `origin/main`. A local working
tree is a snapshot of whenever someone last pulled, so a freshness metric read from it measures
the auditing machine, not the portfolio. The table below is taken from `origin/main` after an
explicit fetch, and the scheduled CI audit -- which clones fresh -- is the authoritative run.

### DOC-010 on `origin/main`, fetched 2026-09-12
...
| kube-ready-box | 10 behind | present | 2026-09-09 |
| ldapium | 5 behind | present | 2026-09-07 |
| kubemetal | 5 behind | present | 2026-08-28 |
| clusterdeck | 7 behind | present | 2026-09-09 |
...
`beluga`, `beluga-manager`, `siqoq` and `egovframe-launcher` have no clone in this workspace.
They were not scored and their status is not guessed here.

A portfolio-wide adoption percentage is deliberately **not** recorded in this section. Scoring only
the repositories that happen to be cloned locally drops the lowest-scoring projects from the
denominator and manufactures an improvement; `docs/portfolio-scorecard.md` is generated by CI
across the full portfolio and remains the only place that number is valid.
