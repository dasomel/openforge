...
""",
    ...
    def write_verification_evidence(self, root: Path) -> None:
        ...
        (evidence_dir / "demo-task.json").write_text(
            json.dumps(
                {
                    ...
                    "happyPath": {
                        "status": "passed",
                        "scenario": "representative happy path",
                        "evidence": ["test:happy"],
                    },
                    "edgeCase": {
                        "status": "passed",
                        "scenario": "representative edge case",
                        "evidence": ["test:edge"],
                    },
                    "deterministicChecks": [
                        {"command": "make verify", "status": "passed", "scope": "repository"}
                    ],
                    "runtimeEvidence": [],
                    "unverified": [],
    ...
    def test_verified_skill_without_evidence_is_false_green(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                any("Agent Skills audit reports" in finding for finding in result["false_green_findings"])
            )
    ...
    def test_verified_skill_with_fresh_session_evidence_is_counted(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(skills["canonical"][0]["evidence_present"])
