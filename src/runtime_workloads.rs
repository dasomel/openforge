...
fn kubectl_json(context: Option<&str>, namespace: Option<&str>, resource: &str) -> Result<Value, String> {
    ...
    if let Some(context) = context { command.arg("--context").arg(context); }
    if let Some(namespace) = namespace { command.arg("-n").arg(namespace); } else { command.arg("-A"); }
...
fn skipped(id: &str, title: &str, reason: String) -> Finding {
    Finding { rule_id: id.into(), category: "Runtime Workload Health".into(), title: title.into(), status: "SKIP", score: 0.0, weight: 0.0, evidence: vec![reason], remediation: String::new() }
}

fn weighted(id: &str, title: &str, ok: usize, total: usize, mut evidence: Vec<String>, remediation: &str) -> Finding {
    if total == 0 { return skipped(id, title, "no applicable resources detected".into()); }
    let coverage = ok as f64 / total as f64;
    evidence.insert(0, format!("coverage={ok}/{total} coverage_percent={:.1}", coverage * 100.0));
    Finding { rule_id: id.into(), category: "Runtime Workload Health".into(), title: title.into(), status: if ok == total { "PASS" } else { "FAIL" }, score: (WEIGHT * coverage * 10.0).round() / 10.0, weight: WEIGHT, evidence, remediation: remediation.into() }
}
...
fn deployment_revisions(items: &[Value]) -> Finding {
    ...
    for d in items {
        ...
        let revision = d.pointer("/metadata/annotations/deployment.kubernetes.io~1revision").and_then(Value::as_str);
        if observed >= generation && revision.is_some() { ok += 1; } else { evidence.push(format!("deployment={} generation={} observed_generation={} revision={}", identity(d), generation, observed, revision.unwrap_or("missing"))); }
    }
...
fn statefulsets(items: &[Value]) -> Finding {
    ...
    for s in items {
        ...
        let observed = s.pointer("/status/observedGeneration").and_then(Value::as_u64).unwrap_or(0);
        let generation = s.pointer("/metadata/generation").and_then(Value::as_u64).unwrap_or(0);
        if ready >= desired && current.is_some() && current == update && observed >= generation { ok += 1; } else { evidence.push(format!("statefulset={} ready={}/{} current_revision={} update_revision={} generation={}/{}", identity(s), ready, desired, current.unwrap_or("missing"), update.unwrap_or("missing"), observed, generation)); }
    }
...
fn jobs(jobs: &[Value], cronjobs: &[Value]) -> Finding {
    ...
    for j in jobs {
        ...
        let completions = j.pointer("/spec/completions").and_then(Value::as_u64).unwrap_or(1);
        let succeeded = j.pointer("/status/succeeded").and_then(Value::as_u64).unwrap_or(0);
        if failed == 0 && (active > 0 || succeeded >= completions) { ok += 1; } else { evidence.push(format!("job={} active={} succeeded={}/{} failed={}", identity(j), active, succeeded, completions, failed)); }
    }
    for c in cronjobs {
        let suspended = c.pointer("/spec/suspend").and_then(Value::as_bool).unwrap_or(false);
        let last_schedule = c.pointer("/status/lastScheduleTime").and_then(Value::as_str);
        let last_success = c.pointer("/status/lastSuccessfulTime").and_then(Value::as_str);
        if suspended || last_success.is_some() || last_schedule.is_none() { ok += 1; } else { evidence.push(format!("cronjob={} suspended={} last_schedule={} last_success=missing", identity(c), suspended, last_schedule.unwrap_or("never"))); }
    }
